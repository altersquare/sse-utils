...
async function runScript() {
	...
	for (let file of files) {
		...
		if (["xlsx", "xls"].includes(file.extension) && file.hasOwnProperty("originalFileUUID")) {
			const newFile = await DBConn[client].collection("mail_meta").findOne({ uuid: file.originalFileUUID });

			if (
				new RegExp(/PYTHON_SCRIPT/gi).test(newFile.remarks) &&
				!new RegExp(/BACKEND_REJECT/gi).test(newFile.remarks)
			) {
				console.log("Python script called");
				file.pdfExtension = newFile.extension;
				file.pdfUUID = newFile.uuid;
				file.pdfFilename = newFile.filename;
				file.pdfMeta = newFile.metaUsed;
			}
		}
		count++;
		console.log(count);
		//get the file parser(helper) for the file extension of the file object provided
		let parser = await getParser(file.pdfExtension ? file.pdfExtension : file.extension);
		...
		if (
			!fs.existsSync(
				projectDir +
					`/data/${EXTENSIONS[file.pdfExtension ? file.pdfExtension : file.extension][0]}/${
						file.pdfUUID || file.uuid
					}_transformed.json`
			)
		) {
			...
			try {
				...
				var readDetails = await parser.read({
					filepath: downloadFilePath + "/" + file.pdfFilename || file.filename,
					timestamp: String(file.pdfUUID || file.uuid),
				});
			} catch (e) {
				console.error(e);
			}
		...
		let meta = await DBConn.sseHQ
			.collection("file_meta")
			.findOne({ _id: file.pdfMeta ? file.pdfMeta : file.metaUsed });
		if (!meta) continue;
		let extractedMeta = await parser.detectMeta(
			{
				originalFilePath: downloadFilePath + "/" + file.pdfFilename || file.filename,
				transformArray,
				uuid: file.pdfUUID || file.uuid,
				execMode: "extract",
			},
			[meta]
		);
